...
@interface JYS_SetTabBar ()

/** 发布按钮 */
@property (nonatomic, strong) UIButton<JYS_PlusButtonSubclassing> *plusButton;

@end

@implementation JYS_SetTabBar
...
- (instancetype)sharedInit {
    if (JYSExternPushlishButton) {
        self.plusButton = JYSExternPushlishButton;
...
-(void)layoutSubviews{
    ...
    CGFloat tabBarButtonW = (CGFloat) barWidth / (JYSTabbarItemsCount + 1);
    ...
    if ([[self.plusButton class] respondsToSelector:@selector(indexOfPlusButtonInTabBar)]) {
        ...
        plusButtonIndex = JYSTabbarItemsCount / 2.0;
    }
